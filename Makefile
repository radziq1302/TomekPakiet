...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "TomekPakiet", "tests")); system.time(test_check("TomekPakiet", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran TomekPakiet_$(VERSION).tar.gz
	rm -rf TomekPakiet.Rcheck/
